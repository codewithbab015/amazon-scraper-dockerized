    ...
    with sync_playwright() as playwright:
        ...
        for idx, product in enumerate(products, 1):
            try:
                ...
                print(f"[{idx}] Failed to extract from {product['link']}: {error}")
    ...
    with sync_playwright() as p:
        ...
        for idx, product in enumerate(products_[:limit_records], 1):
            try:
                ...
                print(f"[{idx}] Failed to extract from {product['link']}: {e}")
...
if __name__ == "__main__":
    ...
    #         filename, cat, sub_cat, products_data_ = get_entries_with_missing_field(
    #             index
    #         )
    #         logger.info("Category Name: %s", cat)
    #         logger.info("SubCategory Name: %s", sub_cat)
    #         logger.info("FileName Name: %s", filename)
    #         if products_data_:
    #             products_all = process_product_detail(
    #                 products=products_data_, subcategory=sub_cat, category=cat
    #             )
    #             # print(json.dumps(data_products_, indent=3))
    #             save_to_json(products_all, f"data/detail_{filename}_v2.json")
    #         else:
    #             logger.info("Empty products list ...")
    #         time.sleep(3)
